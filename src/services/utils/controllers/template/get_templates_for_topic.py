...
def get_templates_for_topic_factory(
    model: Model,
    schema: Schema,
    blueprint: Blueprint,
    ...
    """Creates a Controller for Reading All Instances of Template Model (With Optional Pagination)
    ...
    def get_templates_for_topic(current_user=None):
        args = request.args
        data = model().query.filter_by(topic_id=args.get("topic_id"))
