# -*- coding: utf-8 -*-
"""Module containing the Controller for Reading Triggered Adaptative Events by Template ID.
...
def get_triggered_adaptative_events_by_template_controller_factory(
    ...
    """Creates a Function that Finds all of the adaptative events triggered by the current user for a specific template.
    ...
    def get_triggered_adaptative_events_by_template_controller(
        uuid: str, current_user=None
    ):
        """Function to get all of the triggered adaptative events by the current user for a specific template.
        ...
        """
        template = models["Template"].query.filter_by(id=uuid).first()
        if not template:
            return {
        ...
        # Get all of the adaptative events for the template.
        adaptative_events = template.adaptative_object.adaptative_events.order_by(
            "relative_position"
        ).all()

        # Get other important information about the Template (Parent Topic)
        ...
        return {
            ...
            "message": "Adaptative Events for Template Retrieved Successfully",
        }, 200

    return get_triggered_adaptative_events_by_template_controller
