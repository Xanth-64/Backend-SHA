# -*- coding: utf-8 -*-
"""Module containing the Generic Controller for Reading All Instances of User Model (With Pagination)
...
def get_all_users_with_pagination_controller_factory(
    ...
    """Creates a Controller for Reading All Instances of User Model (With Pagination)
    ...
    def get_all_users_with_pagination_controller(current_user=None):
        args = request.args
        user_type = args.get("user_type") if args.get("user_type") else "student"
        if args.get("sort_key"):
            data = (
                models["User"]()
                .query.join(models["Role"])
                .filter(models["Role"].role_name == user_type)
    ...
    return get_all_users_with_pagination_controller
