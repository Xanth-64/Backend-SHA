...
def update_current_user_controller_factory(
    db: SQLAlchemy,
    model: Model,
    schema: Schema,
    blueprint: Blueprint,
    ...
    @blueprint.route("/current_user_update", methods=["PUT", "PATCH"])
    @auth_middleware(
        expected_role=expected_role, firebase_app=firebase_app, user_model=user_model
    )
    def update_current_user_controller(current_user=None):
        """Update Function for Current User.

        Returns:
            dict: Response dictionary containing the updated model instance.
        """
        data = model().query.get(current_user.id)
        if not data:
            return {
                "success": False,
    ...
    return update_current_user_controller
